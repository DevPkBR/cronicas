...
export default function Home(){
 ...
 async function start(nextMode:'demo'|'ai'){
  ...
  if(!session){setAccount(true);return;}
 ...
 return <div className="app-shell">
  ...
  {!started?<main className="creation">
   ...
   <section className="creation-form">{session&&<section className="saved-campaigns"><h2>Minhas aventuras</h2>{campaigns.length?campaigns.map(c=><button className="secondary-button" key={c.id} disabled={busy} onClick={()=>resume(c.id)}>{c.characters.name} · turno {c.version} · Retomar</button>):<p className="muted">Suas aventuras salvas aparecerão aqui.</p>}</section>}<div className="section-heading"><Feather size={21}/><h2>Quem chega a Breu?</h2></div><p className="muted">Comece pequeno. O resto, você descobre no caminho.</p><form onSubmit={e=>{e.preventDefault();start('ai');}}>
    ...
    <p className="session-note">IA gratuita incluída: entre na sua conta para salvar os turnos automaticamente. Demonstração: 3 cenas fixas, sem salvamento.</p>
   </form></section>
  ...
  <Dialog open={settings} onOpenChange={setSettings}><DialogContent className="game-dialog"><DialogHeader><DialogTitle>Narrador alternativo</DialogTitle><DialogDescription>O jogo já inclui IA gratuita. Uma chave pessoal do Gemini é inteiramente opcional.</DialogDescription></DialogHeader><p>Sem chave pessoal, usamos o Workers AI dentro da infraestrutura do jogo. Se preferir usar sua própria cota do Gemini, configure-a apenas para esta sessão.</p><a className="external-link" href="https://aistudio.google.com/apikey" target="_blank" rel="noreferrer">Google AI Studio ↗</a><label>Chave opcional<input type="password" autoComplete="off" spellCheck={false} value={draftKey} onChange={e=>setDraftKey(e.target.value)} placeholder="Deixe vazio para usar a IA incluída"/></label>{key&&<p className="muted">O Gemini pessoal está configurado nesta sessão.</p>}{error&&<p className="error" role="alert">{error}</p>}<button className="primary-button" disabled={!draftKey.trim()} onClick={configure}>Usar Gemini nesta sessão</button>{key&&<button className="secondary-button" onClick={()=>{setKey('');setDraftKey('');setSettings(false);setError('');}}>Voltar para a IA incluída</button>}<p className="field-note">Chaves pessoais não são salvas no banco. As ações e a memória da história são enviadas ao provedor selecionado; não inclua dados pessoais.</p></DialogContent></Dialog>
